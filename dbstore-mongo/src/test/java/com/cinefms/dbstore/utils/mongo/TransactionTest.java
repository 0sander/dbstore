...
/**
 * Test transaction functionality with graceful handling of MongoDB transaction support
 */
public class TransactionTest extends MongoDataStoreTest {
    ...
    public void itShouldHaveTransactionMethods() {
        ...
        try {
            ...
            String result = mds.executeInTransaction(dataStoreName, () -> "test-result");
        ...
        try {
            ...
            mds.executeInTransaction(dataStoreName, () -> {
        ...
        try {
            ...
            String result = mds.executeInTransaction(dataStoreName, (DBStoreTransactionContext context) -> {
        ...
        try {
            ...
            mds.executeInTransaction(dataStoreName, (DBStoreTransactionContext context) -> {
                // Do nothing - just test the method exists
            });
        } catch (Exception e) {
            // Expected in test environment without transaction support
    ...
    public void itShouldMaintainBackwardCompatibility() {
        ...
        // Test basic CRUD operations - use defaultDb to avoid prefix issues
        SimpleEntity saved = mds.saveObject(null, entity);
        Assert.assertNotNull("Save should work", saved);
        Assert.assertEquals("Value should match", "test", saved.getValue());
        
        SimpleEntity found = mds.getObject(null, SimpleEntity.class, saved.getId());
        Assert.assertNotNull("Get should work", found);
        Assert.assertEquals("Value should match", "test", found.getValue());
        
        // Test field updates
        SimpleEntity updated = mds.setField(null, SimpleEntity.class, saved.getId(), "value", "updated");
        ...
        versioned = mds.saveObject(null, versioned);
        
        VersionedEntity incremented = mds.incrementField(null, VersionedEntity.class, versioned.getId(), "counter", 10);
        ...
        List<SimpleEntity> allEntities = mds.findObjects(null, SimpleEntity.class, null);
        ...
        long count = mds.countObjects(null, SimpleEntity.class, null);
        Assert.assertTrue("Count should be positive", count > 0);
        
        // Test delete operations
        boolean deleted = mds.deleteObject(null, SimpleEntity.class, saved.getId());
        Assert.assertTrue("Delete should work", deleted);
        
        SimpleEntity notFound = mds.getObject(null, SimpleEntity.class, saved.getId());
        Assert.assertNull("Entity should be deleted", notFound);
    }

    @Test
    public void itShouldTestTransactionWithFallback() {
        SimpleEntity entity1 = new SimpleEntity("test1");
        SimpleEntity entity2 = new SimpleEntity("test2");

        try {
            // Try to execute transaction
            List<SimpleEntity> result = mds.executeInTransaction(null, (DBStoreTransactionContext context) -> {
                SimpleEntity saved1 = context.saveObject(entity1);
                SimpleEntity saved2 = context.saveObject(entity2);
                return Arrays.asList(saved1, saved2);
            });

            // If we get here, transactions are supported
            Assert.assertNotNull("Transaction result should not be null", result);
            Assert.assertEquals("Should return 2 entities", 2, result.size());

            // Verify entities were saved
            List<SimpleEntity> allEntities = mds.findObjects(null, SimpleEntity.class, null);
            Assert.assertEquals("Should find 2 entities", 2, allEntities.size());
            
        } catch (Exception e) {
            // If MongoDB doesn't support transactions, test regular operations
            if (e.getMessage().contains("Transaction failed") || 
                e.getMessage().contains("Transactions are not supported")) {
                
                System.out.println("MongoDB transactions not supported in test environment - testing regular operations");
                
                // Test that regular operations still work
                SimpleEntity saved1 = mds.saveObject(null, entity1);
                SimpleEntity saved2 = mds.saveObject(null, entity2);
                Assert.assertNotNull("Regular save should work", saved1);
                Assert.assertNotNull("Regular save should work", saved2);
